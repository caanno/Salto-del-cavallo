    ...
    for row in rows_labels:
        ...
        for col_idx, col in enumerate(cols):
            ...
            if col.button(square_name, key=square_name):
                st.session_state.start_pos = (col_idx, row-1)  # col, row index 0-based

# Inizializza lo stato
if "start_pos" not in st.session_state:
    st.session_state.start_pos = None

# Caso: nessuna casella selezionata
...
# Caso: casella selezionata
    ...
    with col5:
        if st.button("🔄 Scegli un'altra casella"):
            st.session_state.start_pos = None
            st.experimental_rerun()  # opzionale, puoi anche rimuovere e funziona comunque

    # Titolo e casella selezionata
